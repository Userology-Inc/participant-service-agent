...
class TTSSegmentsForwarder:
    ...
    def segment_playout_started(self) -> None:
        ...
        # Record segment start time for word timing and reset words data
        self.segment_start_time = time.time()
        self.words_data = []  # Clear previous word data for the new segment
    ...
    async def _sync_sentence_co(
        ...
        for word in words:
            ...
            # Add the word with timing info to our words_data
            word_end_time = time.time() - segment_start_time
            self.words_data.append({
                'text': word,
                'start': word_start_time,
                'end': word_end_time,
                'confidence': 1.0  # Default confidence for TTS words
            })
